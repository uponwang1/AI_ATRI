...
# ====== DB / Upload 設定 ======
DB_FILE = "weather_1021.db"   # 分析用（日資料）
CWB_DB  = "CWB.db"            # 即時用（逐時）
UPLOAD_FOLDER = "uploads"
os.makedirs(UPLOAD_FOLDER, exist_ok=True)


# ---------------- 初始化分析資料庫（weather_1021.db） ----------------
def init_db():
...
def process_csv(filepath, filename):
    ...
    print(f"✅ 匯入完成 {filename}（新增 {len(new_data)} 筆，原始 {len(df_new)} 筆）")


# ---------------- 主頁（新首頁：wmindex.html_v1） ----------------
...
def index():
    ...
    return render_template("wmindex.html_v1", data=data)
...
def gdd_compare():
    ...
    if df.empty:
        return jsonify({"table": [], "best": {"Tb": 0, "std": 0}})
    ...
    for Tb in range(0, 21):
        ...
        def calc_gdd(start, end):
            ...
            return round(float(gdd), 2)

        g1 = calc_gdd(r1[0], r1[1])
        g2 = calc_gdd(r2[0], r2[1])
        g3 = calc_gdd(r3[0], r3[1])
        std = float(np.std([g1, g2, g3]))
        results.append({"Tb": Tb, "GDD1": g1, "GDD2": g2, "GDD3": g3, "std": round(std, 2)})

    best = min(results, key=lambda x: x["std"]) if results else {"Tb": 0, "std": 0}
    return jsonify({"table": results, "best": best})


# ---------------- 即時逐時頁：讀 CWB.db → indexCWB.html_v8.1 ----------------
@app.route("/realtime")
def realtime():
    # 如果 CWB.db 不存在或 table 不存在，至少不要讓首頁掛掉
    labels, temps, hums = [], [], []
    try:
        with sqlite3.connect(CWB_DB) as conn:
            df = pd.read_sql_query(
                "SELECT obs_date, temperature, humidity FROM weather ORDER BY obs_date ASC",
                conn
            )
        if not df.empty:
            labels = df["obs_date"].astype(str).tolist()
            temps = df["temperature"].tolist()
            hums = df["humidity"].tolist()
    except Exception as e:
        print(f"⚠️ 讀取 CWB.db 失敗：{e}")

    # 直接沿用你既有的 indexCWB.html_v8.1（不需要改名）
    return render_template("indexCWB.html_v8.1", labels=labels, temps=temps, hums=hums)


if __name__ == "__main__":
    # 主站建議用 5000（跟你原本 weather8.1.py 一樣）
